...
class Cosine_PDG_Adam:
    ...
    def step(self, image_min, image_max, image, prediction, target):
        ...
        loss = self.loss_function(prediction, target).sum()
...
def model_immer_attack_auto_loss(image, model, attack, number_of_steps, device):
    ...
    target = model(image)
    target_inner = torch.rand(*target.shape).to(device)

    for i in range(number_of_steps):
        prediction = model(image_adv)
        image_adv = attack.step(image_min, image_max, image_adv, prediction, target_inner)
        model.zero_grad()
