...
save_folder_name = "bim_full_cira+2"

CONFIG_PATH_MAIN = "../configs/config_main.json"
CONFIG_MAIN = json.load(open(CONFIG_PATH_MAIN))

try:
    print(CONFIG_MAIN['SAVE_FOLDER'] + save_folder_name + "/")
    os.mkdir(CONFIG_MAIN['SAVE_FOLDER'] + save_folder_name + "/")
...
for e, (images, labels, label) in enumerate(val_loader):
    ...
    if(-1 < e):
        ...
        torch.save(pred_sum_mask, CONFIG_MAIN['SAVE_FOLDER'] + save_folder_name + "/prediction_" + str(e) + ".pth")
        torch.save(label[0], CONFIG_MAIN['SAVE_FOLDER'] + save_folder_name + "/label_" + str(e) + ".pth")
